...
The processor package manages RedScope stack lifecycle through state-oriented approach. Main idea: maintain consistency between desired user state (state.State) and actual system state (checker.CheckResult).

## Key Principles

1. **State-Driven Operations**: All operations based on comparing current and target state
2. **Stack Independence**: Each stack (observability, langfuse, redscope) managed independently
...
#### Phase 3: RedScope Stack Management
```go
// RedScope always embedded, always required
...
**Rationale**: RedScope - main stack, always installed, only file integrity check.
...
- dependency ordering: observability → langfuse → redscope
