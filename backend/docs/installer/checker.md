...
The `checker` package is responsible for gathering system facts and verifying installation prerequisites for RedScope. It performs comprehensive system analysis to determine the current state of the installation and what operations are available.
...
- Installation status for each component (RedScope, Langfuse, Observability)
...
- **Script Installation**: Verifies RedScope CLI script in /usr/local/bin
...
- Checks updates for: Installer, RedScope, Langfuse, Observability, Worker images
