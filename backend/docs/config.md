# RedScope Configuration Guide

This document serves as a comprehensive guide to the configuration system in RedScope, primarily aimed at developers. It details all available configuration options, their purposes, default values, and how they're used throughout the application.

## Table of Contents

- [RedScope Configuration Guide](#redscope-configuration-guide)
...
RedScope uses environment variables for configuration, with support for `.env` files through the `godotenv` package. The configuration is defined in the `Config` struct in `pkg/config/config.go` and is loaded using the `NewConfig()` function.
...
| -------------- | -------------------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| DatabaseURL    | `DATABASE_URL`       | `postgres://redscopeuser:redscopepass@pgvector:5432/redscopedb?sslmode=disable` | Connection string for the PostgreSQL database with pgvector extension    |
...
| InstallationID | `INSTALLATION_ID`    | *(none)*                                                                     | Unique installation identifier for RedScope Cloud API communication       |
| LicenseKey     | `LICENSE_KEY`        | *(none)*                                                                     | License key for RedScope Cloud API authentication and feature activation  |
...
- **DataDir**: Specifies where RedScope stores persistent data. This is used across multiple components:
...
- **InstallationID**: A unique identifier for the RedScope installation used for cloud API communication:
...
- **LicenseKey**: Authentication key for RedScope Cloud API and premium feature activation:
  - Validates license and enables licensed features
  - Required for enterprise features and support
  - Used for authentication with RedScope Cloud services
...
These settings control how RedScope interacts with Docker, which is used for terminal isolation and executing commands in a controlled environment. They're crucial for the security and functionality of tool execution.

| Option                       | Environment Variable               | Default Value          | Description |
| ---------------------------- | ---------------------------------- | ---------------------- | ----------- |
| DockerInside                 | `DOCKER_INSIDE`                    | `false`                | Set to `true` if RedScope runs inside Docker and needs to access the host Docker daemon. |
...
- **DockerInside**: Signals whether RedScope is running inside a Docker container itself, which affects how volumes and sockets are mounted:
  ```go
...
- **DockerNetwork**: Controls the network isolation mode for containers. Supports two modes:
  ...
  **Bridge Mode** (custom network name, e.g., `redscope-network`):
...
These settings control the HTTP and GraphQL server that forms the backend API of RedScope.
...
**LiteLLM Integration**: Set `DEEPSEEK_PROVIDER=deepseek` to enable model prefixing (e.g., `deepseek/deepseek-chat`) when using LiteLLM proxy with default RedScope configs.
...
**LiteLLM Integration**: Set `GLM_PROVIDER=zai` to enable model prefixing (e.g., `zai/glm-4`) when using LiteLLM proxy with default RedScope configs.
...
**LiteLLM Integration**: Set `KIMI_PROVIDER=moonshot` to enable model prefixing (e.g., `moonshot/kimi-k2.5`) when using LiteLLM proxy with default RedScope configs.
...
**LiteLLM Integration**: Set `QWEN_PROVIDER=dashscope` to enable model prefixing (e.g., `dashscope/qwen-plus`) when using LiteLLM proxy with default RedScope configs.
...
These settings control the vector embedding service used for semantic search and similarity matching, which is fundamental for RedScope's intelligent search capabilities.
