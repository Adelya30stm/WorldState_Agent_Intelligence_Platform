...
The Ollama provider enables RedScope to use local language models through the [Ollama](https://ollama.ai/) server.
...
Control how RedScope interacts with your Ollama server:
...
**Performance Note:** Enabling `OLLAMA_SERVER_LOAD_MODELS_ENABLED` adds startup latency as RedScope queries the Ollama API. Disable if you only need specific models from config file.
...
The provider supports all RedScope agent types with optimized configurations:
...
# Start RedScope with Ollama provider
./redscope
```
