...
- [Integration with RedScope](#integration-with-redscope)
...
The Docker client package (`backend/pkg/docker`) provides a secure and isolated containerized environment for RedScope's AI agents to execute penetration testing operations. This package serves as a wrapper around the official Docker SDK, offering specialized functionality for managing containers that AI agents use to perform security testing tasks.
...
### Role in RedScope Ecosystem

The Docker client is a critical component that enables RedScope's core promise of secure, isolated penetration testing. It provides the foundation for:
...
RedScope uses a deterministic port allocation algorithm to ensure each flow gets unique, predictable ports:
...
| `DOCKER_INSIDE` | `false` | Whether RedScope communicates with host Docker daemon from containers |
| `DOCKER_NET_ADMIN` | `false` | Whether RedScope grants the primary container NET_ADMIN capability for advanced networking. |
...
The `DOCKER_NET_ADMIN` option controls whether RedScope containers are granted the `NET_ADMIN` Linux capability, which provides advanced networking permissions essential for many penetration testing operations.
...
RedScope supports running inside Docker containers while still managing other containers. This is controlled by the `DOCKER_INSIDE` setting:

- **`DOCKER_INSIDE=false`**: RedScope runs on host, manages containers directly
- **`DOCKER_INSIDE=true`**: RedScope runs in container, mounts Docker socket to manage sibling containers

### Network Configuration

RedScope supports two network modes for container isolation:

#### Bridge Network Mode (Default)

When `DOCKER_NETWORK` is set to a custom network name (e.g., `redscope-network`), containers are connected to an isolated bridge network:
...
The main interface defines all Docker operations available to RedScope components:
...
RedScope tracks container states in the database:
...
func PrimaryTerminalName(flowID int64) string {
    return fmt.Sprintf("redscope-terminal-%d", flowID)
}
```

This creates names like `redscope-terminal-123` for flow ID 123, making it easy to:
...
RedScope implements a multi-layered security approach for container isolation:
...
## Integration with RedScope

### Tool Integration

The Docker client integrates with RedScope's tool system to provide terminal access:
...
- Integrate with RedScope's database for state management
