# Langfuse Integration for RedScope

This document provides a comprehensive guide to the Langfuse integration in RedScope, covering architecture, setup, usage patterns, and best practices for developers.

## Table of Contents

- [Langfuse Integration for RedScope](#langfuse-integration-for-redscope)
...
Langfuse is an open-source observability platform specifically designed for LLM-powered applications. The RedScope integration with Langfuse provides:
...
The Langfuse integration in RedScope is built around a layered architecture that provides both high-level abstractions for simple use cases and fine-grained control for complex scenarios.

```mermaid
flowchart TD
    Application[RedScope Application] --> Observer[Observer]
...
```mermaid
sequenceDiagram
    participant App as RedScope Application
...
| `LANGFUSE_INIT_USER_EMAIL` | Admin user email | admin@redscope.com |
...
import (
    "redscope/pkg/observability/langfuse"
    "redscope/pkg/config"
)
...
The main integration point in RedScope is the Flow Controller, which handles the lifecycle of AI agent flows:
