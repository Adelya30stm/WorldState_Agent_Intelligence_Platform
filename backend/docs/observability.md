# RedScope Observability Stack

## Table of Contents
- [RedScope Observability Stack](#redscope-observability-stack)
...
The RedScope Observability Stack provides comprehensive monitoring, logging, tracing, and metrics collection for the application. It integrates multiple technologies to provide a complete view of the application's behavior, performance, and health:
...
```mermaid
flowchart TD
    App[RedScope Application] --> Observer[Observability Interface]
...
```mermaid
sequenceDiagram
    participant App as RedScope App
...
RedScope uses several patterns for instrumenting different parts of the application. These patterns demonstrate best practices for integrating the observability stack.

### HTTP Server Instrumentation

The RedScope application uses a Gin middleware to instrument HTTP requests, located in `pkg/server/logger/logger.go`:
...
RedScope also provides instrumentation for GraphQL operations:
