...
const (
	...
	WelcomeFormTitle       = "Welcome to RedScope"
	WelcomeFormDescription = "RedScope is an autonomous penetration testing platform that leverages AI technologies to perform comprehensive security assessments."
...
The installer guides you through each component setup with recommendations for different deployment scenarios.`
	...
	WelcomeWorkflowStep5 = "5. Deploy and start RedScope services"
...
const (
	...
	TroubleshootEnvFileDesc  = "The .env file is required for RedScope configuration but was not found or is not readable."
...
4. Ensure sufficient disk space is available`
	...
	TroubleshootDockerNotInstalledDesc  = "Docker is not installed on this system. RedScope requires Docker to run containers."
...
5. Try: export DOCKER_HOST=unix:///var/run/docker.sock`
	...
	TroubleshootDockerVersionDesc  = "Your Docker version is incompatible. RedScope requires Docker 20.0.0 or newer."
...
Verify installation: docker compose version`
	...
	TroubleshootComposeVersionDesc  = "Your Docker Compose version is incompatible. RedScope requires Docker Compose 1.25.0 or newer."
...
5. Ensure certificates are valid if using TLS`
	...
	TroubleshootCPUDesc  = "RedScope requires at least 2 CPU cores for proper operation."
...
const (
	// Form interface implementation
	EULAFormDescription = "Legal terms and conditions for RedScope usage"
	EULAFormName        = "EULA"
	EULAFormOverview    = `Review and accept the End User License Agreement to proceed with RedScope installation.
...
const (
	MainMenuTitle       = "RedScope Configuration"
	MainMenuDescription = "Configure all RedScope components and settings"
	MainMenuName        = "Main Menu"
	MainMenuOverview    = `Welcome to RedScope Configuration Center.
...
Navigate through each section to complete your RedScope setup.`
...
const (
	MainMenuStatusPentagiRunning     = "RedScope is already running"
	MainMenuStatusPentagiNotRunning  = "Ready to start RedScope services"
	MainMenuStatusUpToDate           = "RedScope is up to date"
...
const (
	...
	LLMProvidersOverview    = `RedScope uses specialized AI agents (researcher, developer, executor, pentester) that require different LLM capabilities for optimal penetration testing results.
...
Ready-to-use configurations for OpenRouter, DeepInfra, vLLM, Ollama, and other providers are available in the /opt/redscope/conf/ directory inside the container`
)
...
Examples available: Pre-configured setups for major providers in /opt/redscope/conf/ directory inside the container`
)
...
Monitor RedScope performance and system health.`
...
const (
	...
	ServerSettingsFormDescription = "Configure RedScope server network access and public routing"
	ServerSettingsFormName        = "Server Settings"
	ServerSettingsFormOverview    = `• Network binding - control which interface and port RedScope listens on
...
• Data directory - persistent storage for agent artifacts and flow workspaces`
	...
	ServerSettingsLicenseKeyDesc = "RedScope License Key in format of XXXX-XXXX-XXXX-XXXX"
	...
	ServerSettingsPortDesc = "External TCP port exposed by Docker for RedScope web UI"

	ServerSettingsPublicURL     = "Public URL"
	ServerSettingsPublicURLDesc = "Base public URL for redirects and links (supports base path, e.g., https://example.com/redscope/)"
	...
	ServerSettingsExternalSSLCAPathDesc = "Path inside container to custom root CA cert (e.g., /opt/redscope/ssl/ca-bundle.pem)"
	...
	ServerSettingsGeneralHelp = `RedScope exposes its web UI via Docker with configurable host and port.

Public URL must reflect how users reach the server. If using a subpath (e.g., /redscope/), include it here. CORS controls browser access from specified origins. Proxy affects outbound traffic to LLM/search providers and other external services used by Tools.
...
SSL directory allows providing custom certificates. When set, server will use server.crt and server.key from that directory. Data directory stores artifacts and working files for flows.`
	...
	ServerSettingsLicenseKeyHelp = `RedScope License Key in format of XXXX-XXXX-XXXX-XXXX. It's used to communicate with RedScope Cloud API.`
...
• 0.0.0.0 — expose on all interfaces`
	...
	ServerSettingsPortHelp = `External port for RedScope UI. Must be available on the host. Example: 8443.`
...
• http://localhost:8443
• https://example.com/
• https://example.com/redscope/ (with base path)`
...
Must point to /opt/redscope/ssl/ directory, which is mounted from redscope-ssl volume on the host.

Examples:
• /opt/redscope/ssl/ca-bundle.pem
• /opt/redscope/ssl/corporate-ca.pem
...
When enabled, all certificate validation is bypassed, making connections vulnerable to man-in-the-middle attacks.`
	...
	ServerSettingsDataDirHelp = `Host directory for persistent data. RedScope stores agent artifacts under flow-N subdirectories, which map to /work inside worker containers.`
...
const (
	...
	ToolsAIAgentsSettingsFormOverview    = `This section configures global behavior of AI agents across RedScope.
...
Examples:
• 'redscope-network' - creates isolated bridge network
...
Change providers carefully - different embedders produce incompatible vectors requiring database reindexing.`
	...
	EmbedderHelpGeneral = `Embeddings convert text into vectors for semantic search and knowledge storage. This enables RedScope to understand meaning rather than just keywords, making search results more relevant and intelligent.
...
const (
	...
	ApplyChangesFormOverview = `This screen allows you to review all pending configuration changes and apply them to your RedScope installation.
...
• Install additional components if needed`
	...
	ApplyChangesInstallNotFound = `RedScope is not currently installed on this system.

The following actions will be performed:
• Docker environment setup and validation
• Creation of docker-compose.yml file
• Installation and startup of RedScope core services`
	...
	ApplyChangesUpdateFound = `RedScope is currently installed on this system.
...
const (
	...
	MaintenanceDescription = "Manage RedScope services and perform maintenance operations"
	MaintenanceName        = "Maintenance"
	MaintenanceOverview    = `Perform system maintenance operations for RedScope.
...
Each operation will provide real-time status updates and confirmation when required.`
	...
	MaintenanceStartPentagi            = "Start RedScope"
	MaintenanceStartPentagiDesc        = "Start all configured RedScope services"
	MaintenanceStopPentagi             = "Stop RedScope"
	MaintenanceStopPentagiDesc         = "Stop all running RedScope services"
	MaintenanceRestartPentagi          = "Restart RedScope"
	MaintenanceRestartPentagiDesc      = "Restart all RedScope services"
	...
	MaintenanceUpdatePentagi           = "Update RedScope"
	MaintenanceUpdatePentagiDesc       = "Update RedScope to the latest version"
	...
	MaintenanceFactoryResetDesc        = "Reset RedScope to factory defaults"
	MaintenanceRemovePentagi           = "Remove RedScope"
	MaintenanceRemovePentagiDesc       = "Remove RedScope containers but keep data"
	MaintenancePurgePentagi            = "Purge RedScope"
	MaintenancePurgePentagiDesc        = "Completely remove RedScope including all data"
	MaintenanceResetPassword           = "Reset Admin Password"
	MaintenanceResetPasswordDesc       = "Reset the administrator password for RedScope"
)

// Reset Password Screen constants
const (
	ResetPasswordFormTitle       = "Reset Admin Password"
	ResetPasswordFormDescription = "Reset the administrator password for RedScope"
	ResetPasswordFormName        = "Reset Password"
	ResetPasswordFormOverview    = `Reset the password for the default administrator account (admin@admin.com).

This operation requires RedScope to be running and will update the password in the PostgreSQL database.
...
• Both password fields must match`
	...
	// Status messages
	ResetPasswordNotAvailable = "RedScope must be running to reset password"
	...
	// Help content
	ResetPasswordHelpContent = `Reset the administrator password for accessing RedScope.

This operation:
• Updates the password for admin@admin.com account
• Sets the user status to 'active'
• Requires RedScope database to be accessible
...
const (
	...
	ProcessorOperationInstalling  = "Installing RedScope services..."
	...
	OperationTitleInstallPentagi    = "Install RedScope"
	OperationDescInstallPentagi     = "Install and configure RedScope services"
...
• Core RedScope API and web interface
...
const (
	...
	EnvDesc_LICENSE_KEY                       = "RedScope License Key"
	EnvDesc_REDSCOPE_LISTEN_IP                 = "RedScope Server Host"
	EnvDesc_REDSCOPE_LISTEN_PORT               = "RedScope Server Port"
	EnvDesc_PUBLIC_URL                        = "RedScope Public URL"
	EnvDesc_CORS_ORIGINS                      = "RedScope CORS Origins"
	EnvDesc_COOKIE_SIGNING_SALT               = "RedScope Cookie Signing Salt"
	...
	EnvDesc_REDSCOPE_SSL_DIR                   = "RedScope SSL Directory"
	EnvDesc_REDSCOPE_DATA_DIR                  = "RedScope Data Directory"
	EnvDesc_REDSCOPE_DOCKER_SOCKET             = "Mount Docker Socket Path"
	EnvDesc_REDSCOPE_DOCKER_CERT_PATH          = "Mount Docker Certificate Path"
	EnvDesc_REDSCOPE_LLM_SERVER_CONFIG_PATH    = "Custom LLM Host Config Path"
	EnvDesc_REDSCOPE_OLLAMA_SERVER_CONFIG_PATH = "Ollama Host Config Path"
	...
	EnvDesc_REDSCOPE_POSTGRES_PASSWORD = "RedScope PostgreSQL Password"
...
const (
	...
	// component labels
	ProcessorComponentPentagi       = "RedScope"
	...
	EffectsStart           = "RedScope web UI becomes available. Background services are brought online in the required order."
	EffectsStop            = "Web UI becomes unavailable. In-progress flows pause safely. When you start RedScope again, flows resume automatically. A small portion of the current agent step may be lost."
